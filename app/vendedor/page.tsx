...
import { Store, Package, TrendingUp, DollarSign, Loader2, Minus, Plus, ChevronDown, ChevronUp } from "lucide-react";
...
export default function VendedorDashboard() {
  ...
  return (
    <div>
      ...
      {/* Inventory per-book cards */}
      <div className="bg-white/5 border border-white/8 rounded-2xl overflow-hidden mb-8">
        <div className="px-5 py-4 border-b border-white/8 flex items-center justify-between">
          <h2 className="font-semibold flex items-center gap-2">
            <Package className="w-4 h-4 text-amber-400" />
            Inventario ({activeInventory.length + zeroStockInventory.length} títulos)
          </h2>
          <span className="text-xs text-white/30">{totalStock} unidades en stock</span>
        </div>
